# Tasks: Runtime Settings CLI Validation Host Runtime Observation Adapter
...
observation adapter implementation unit.

**Organization**: Tasks are grouped into public import/spec/admission closeout,
the admitted host runtime observation IAU, blocked future runtime/release/locator
...
host runtime observation facts.
...
- [x] T003 Create `spec.md` for bounded host runtime observation facts and
...
## Phase 2: Admitted IAU - Host Runtime Observation Facts

**Purpose**: Implement the smallest observation step before host runtime
discovery: tests and a bounded observation-facts contract that emits
public-safe discovery observation facts without execution.
...
- [ ] T017 [BLOCKED] Retain raw private installed paths or raw registry output
  in public facts.
- [ ] T018 [BLOCKED] Add arbitrary filesystem walking beyond the admitted
...
- [ ] T026 [BLOCKED] Expand proof-out behavior beyond existing contracts or
  write files from the observation adapter.
...
- [ ] T028 [BLOCKED] Add source copying from another VI History product line
  or implementation beyond host runtime observation without a separate public
  IAU.
...
- `runtime-settings-cli-validation-host-runtime-discovery-v1` was implemented
  and closed before
  `IAU-runtime-settings-cli-validation-host-runtime-observation-adapter-v1`
  admission started.
...
- Phase 3 remains blocked until separate public requirements and preflight
  ...
  execution, terminal process wiring, live proof, proof-out expansion, file
  writes, package/bin publication, launcher/profile mutation, release
  automation, Marketplace publication, or source copying.
- Issue #130 is the admission issue and must not be reused for implementation.
...
4. Keep runtime validation execution, compare execution, LabVIEWCLI execution,
   ...
   automation, Marketplace publication, and source copying blocked.
