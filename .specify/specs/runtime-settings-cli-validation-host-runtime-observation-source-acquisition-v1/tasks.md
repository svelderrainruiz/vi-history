# Tasks: Runtime Settings CLI Validation Host Runtime Observation Source Acquisition
...
observation source acquisition implementation unit.

**Organization**: Tasks are grouped into public import/spec/admission closeout,
the admitted host runtime observation source acquisition IAU, blocked future
...
bounded native host source acquisition facts.
...
- [x] T003 Create `spec.md` for bounded host runtime observation source
  acquisition facts and blocked runtime execution/release scope.
...
## Phase 2: Admitted IAU - Host Runtime Observation Source Acquisition

**Purpose**: Implement the smallest native source-acquisition step before host
runtime observation source adaptation: tests and a bounded acquisition contract
that emits public-safe source facts without execution.
...
- [ ] T026 [BLOCKED] Expand proof-out behavior beyond existing contracts or
  write files from the source acquisition, source, observation, discovery, or
  preflight adapters.
...
- [ ] T028 [BLOCKED] Add source copying from another VI History product line
  or implementation beyond host runtime observation source acquisition without
  a separate public IAU.
- [ ] T029 [BLOCKED] Add direct native registry, filesystem, PATH, environment,
  or runtime locator probes outside an admitted bounded dependency surface.
- [ ] T030 [BLOCKED] Add Docker image inspection or container source discovery.
...
- `runtime-settings-cli-validation-host-runtime-observation-source-adapter-v1`
  was implemented and promoted before source-acquisition admission started.
...
- Phase 3 remains blocked until separate public requirements and preflight
  ...
  source copying.
- Issue #148 is the admission issue and must not be reused for implementation.
