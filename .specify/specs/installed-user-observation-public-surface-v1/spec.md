...
**Status**: Locked for public import; observation-model IAU implemented
...
- **SC-004**: The admission record states that
  `IAU-installed-user-observation-model-v1` implemented `T009` through `T013`,
  and that future code outside that scope requires a separate named IAU
  preflight with `status: pass`.
...
- This feature is a public observation and reporting model first. Runnable
  implementation is complete only for the observation-model IAU.
