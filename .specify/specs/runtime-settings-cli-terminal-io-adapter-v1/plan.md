# Implementation Plan: Runtime Settings CLI Terminal I/O Adapter

**Branch**: `codex/runtime-settings-cli-terminal-io-adapter-admission` |
...
Import and lock the runtime settings CLI terminal I/O adapter contract for the
MIT clean-room Spec Kit authority. The feature defines the next narrow
implementation admission unit after terminal prompt-loop closeout:
...
records, validation script coverage, and Issue #77 as the public admission
...
records, and future public terminal I/O adapter contracts.
...
expansion, launcher/profile mutation, or live terminal proof is required for
this admission.
...
Marketplace publication remains disabled. Terminal I/O adapter admission does
not imply compare execution, runtime execution, proof-out expansion,
live-session proof, package/bin publication, launcher/profile mutation, or
...
docs/requirements/imports/runtime-settings-cli-terminal-io-adapter-v1/
├── manifest.json
├── syrs.md
├── srs.md
├── rtm.csv
...
├── runtime-settings-cli-terminal-io-adapter-v1.json
└── runtime-settings-cli-terminal-io-adapter-v1.md
```
...
The governed bridge-readiness record admits the terminal I/O adapter candidate
only after the materialized-entrypoint and terminal prompt-loop IAUs were
implemented and closed. The adapter IAU may bind already public entrypoint,
interactive-selection, and prompt-loop facts to terminal-session input/output
facts for no-argument `vihs`.

The record keeps compare execution, LabVIEWCLI execution, Docker command
execution or orchestration, proof expansion, live-session proof, package/bin
publication, launcher/profile mutation, Marketplace work, and source copying
outside this IAU.
...
- interactive Enter confirmation adapts terminal input facts into the existing
  prompt-loop validation handoff
- guided host selection feeds the already admitted host selection contract
...
- non-interactive sessions receive exact copyable next-command guidance instead
  of a hanging prompt
- EOF, cancel, unsupported input, unsupported image-family requests, and Docker
  bitness-choice requests fail closed without side effects
- validation handoff reuses the public `vihs --validate` readback contract
- no compare execution, LabVIEWCLI execution, Docker execution, proof
  expansion, package/bin publication, launcher/profile mutation, Marketplace
  behavior, or source copying is started
